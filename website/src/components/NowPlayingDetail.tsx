  ...
  useEffect(() => {
    const updateElapsed = async () => {
      ...
      if (activity.timestamp > lastUpdated) {
        setElapsed(+activity.elapsed + (Date.now() - activity.timestamp));
    ...
    const v = Vibrant.from(activity.imageUrl).useQuantizer(
      Vibrant.Quantizer.WebWorker || Vibrant.Quantizer.MMCQ,
    );
  ...
  return (
    <div className="h-full w-full text-left dark:text-white">
      <div className="clear-both mb-2">
        ...
        />
        <p className="my-2 inline text-gray-600 dark:text-gray-400"> – Plex</p>
      </div>
      {activity.imageUrl ? (
